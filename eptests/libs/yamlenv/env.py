...
from eptests.libs.yamlenv import Cache, Obj, ObjIFunc, Path, WalkType
...
def objwalk(obj, path=None, memo=None):
    ...
    elif isinstance(obj, (Sequence, Set)) and not isinstance(obj, six.string_types):
        iterator = enumerate
    if iterator:
...
/Shell-Parameter-Expansion.html
    ...
    RE = re.compile(r"\$\{(?P<name>[^:-]+?)((?P<separator>:?)-(?P<default>.*?))?\}")
